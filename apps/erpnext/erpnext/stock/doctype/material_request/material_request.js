...
frappe.ui.form.on("Material Request", {
	...
	refresh: function (frm) {
		...
		frm.events.update_custom_card_options(frm);
	},
	...
	custom_package: function (frm) {
		frm.set_value("custom_card", "");
		frm.events.update_custom_card_options(frm);
	},

	update_custom_card_options: function (frm) {
		if (!frm.fields_dict.custom_card) {
			return;
		}

		const pack = frm.doc.custom_package;

		if (!pack) {
			frm.set_df_property("custom_card", "options", [""]);
			frm.set_value("custom_card", "");
			return;
		}

		frappe.call({
			method: "ursc_cms_app.api.get_card_names_for_material_request",
			args: { pack_code: pack },
			callback: (r) => {
				const options = (r.message || []).filter(Boolean);
				frm.set_df_property("custom_card", "options", ["", ...options]);
				if (!options.includes(frm.doc.custom_card)) {
					frm.set_value("custom_card", "");
				}
				frm.refresh_field("custom_card");
			},
		});
	},
