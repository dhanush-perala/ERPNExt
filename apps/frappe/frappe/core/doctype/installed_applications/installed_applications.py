...
# You can add this either to installed_applications.py or directly in activation.py

def get_setup_wizard_completed_apps():
    """Get apps for which setup wizard has been completed."""
    completed_apps = []

    if frappe.db.exists("Installed Application"):
        apps = frappe.get_all("Installed Application", fields=["app_name"])
        completed_apps = [app["app_name"] for app in apps]
