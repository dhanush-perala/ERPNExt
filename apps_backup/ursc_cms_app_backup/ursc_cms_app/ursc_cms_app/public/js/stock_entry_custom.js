...
function update_base_and_mdb(frm, cdt, cdn) {

  const row = locals[cdt][cdn];

  if (!row.item_code) return;
 
  const supplier = frm.doc.supplier || row.supplier || null;
 
  frappe.call({

    method: "ursc_cms_app.api.get_basic_part_number",

    args: { item_code: row.item_code, supplier },
    ...
    callback: (r) => {
      ...
      frappe.model.set_value(cdt, cdn, "custom_base_part_no", base);
 
      const qual = row.custom_quality_save || "";

      frappe.model.set_value(cdt, cdn, "custom_mdb_part_no", `${qual}${base}`);
...
frappe.ui.form.on("Stock Entry Detail", {
  ...
  item_code(frm, cdt, cdn) {
    ...
    update_base_and_mdb(frm, cdt, cdn);
  ...
  custom_quality_save(frm, cdt, cdn) {
    ...
    const base = row.custom_base_part_no || "";
...
frappe.ui.form.on("Stock Entry", {
  ...
  supplier(frm) {
    ...
    (frm.doc.items || []).forEach((d) => {
      ...
      update_base_and_mdb(frm, d.doctype, d.name);
  ...
  onload(frm) {
    ...
    (frm.doc.items || []).forEach((d) => {
      ...
      if (!d.custom_base_part_no && d.item_code) {
